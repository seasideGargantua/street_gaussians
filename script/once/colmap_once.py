...
image_filename_to_cam = lambda x: int(_cam_name_id_map[x.split('/')[-2]]) # .../{cam_name}/{frame}.png

def convert_filename(cam_name, basename):
    return f'{cam_name}/{basename}'

def run_colmap_once(result):    
    model_path = cfg.model_path
    data_path = os.path.join(cfg.source_path, cfg.data.seq_id)
    ...
    for unqiue_cam in unique_cams:
        cam_name = _cam_id_name_map[unqiue_cam]
        train_images_dir = os.path.join(colmap_dir, 'train_imgs', f'{cam_name}')
        test_images_dir = os.path.join(colmap_dir, 'test_imgs', f'{cam_name}')
        mask_images_dir = os.path.join(colmap_dir, 'mask', f'{cam_name}')
    ...
    for i, image_filename in enumerate(image_filenames):
        ...
        cam_name = _cam_id_name_map[cams[i]]
        basename = os.path.basename(image_filename)
        new_image_filename = convert_filename(cam_name, basename)
        c2w_dict[new_image_filename] = c2ws[i]
        mask_image_filenames.append(os.path.join(data_path, 'dynamic_mask', cam_name, basename))
    ...
    for i, image_filename in enumerate(train_image_filenames):
        basename = os.path.basename(image_filename)
        cam_name = _cam_id_name_map[cams[i]]
        new_image_filename = os.path.join(train_images_dir, convert_filename(cam_name, basename))
    ...
    for i, image_filename in enumerate(test_image_filenames):
        basename = os.path.basename(image_filename)
        cam_name = _cam_id_name_map[cams[i]]
        new_image_filename = os.path.join(test_images_dir, convert_filename(cam_name, basename))
        if not os.path.exists(new_image_filename):
            shutil.copyfile(image_filename, new_image_filename)
    ...
    for i, image_filename in enumerate(mask_image_filenames):
        ...
        cam_name = _cam_id_name_map[cams[i]]
        mask_images_dir = os.path.join(colmap_dir, 'mask')
        new_image_filename = os.path.join(mask_images_dir, convert_filename(cam_name, basename))
    ...
    ref_camera_name = _cam_id_name_map[ref_camera_id]
    ...
    for cam_id in unique_cams:
        ...
        cur_camera_name = _cam_id_name_map[cam_id]

        ref_extrinsic = np.array(calib[ref_camera_name]['cam_to_velo'])
        cur_extrinsic = np.array(calib[cur_camera_name]['cam_to_velo'])
